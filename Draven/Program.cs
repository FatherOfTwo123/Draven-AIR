...
namespace Draven
{
    ...
    public static class Program
    {
        ...
        public static string GameClientRootEnvVar = "DRAVEN_GAME_CLIENT_ROOT";
        public static string LeagueDrive = ResolveGameClientRoot();
        public static string[] AuthLocations = new string[] { "http://127.0.0.1:8080/" };

        private static string ResolveGameClientRoot()
        {
            string rawPath = Environment.GetEnvironmentVariable(GameClientRootEnvVar);
            if (string.IsNullOrWhiteSpace(rawPath))
                return string.Empty;

            try
            {
                return Path.GetFullPath(rawPath).Replace('\\', '/');
            }
            catch
            {
        ...
        private static readonly List <string> Services = new List <string>
        {
            ...
            "PlayerStatsService", "GameService"
        };
        ...
        static void Main(string[] args)
        {
            ...
            var certificateStore = new X509Store(StoreName.TrustedPeople, StoreLocation.CurrentUser);
        ...
        static void ClientCommandReceieved(object sender, CommandMessageReceivedEventArgs e)
        {
            ...
            if (e.Message.Operation == CommandOperation.Login)
            {
                ...
                clientConfig.Config = clientConfig.Config
                    .Replace("matchhistory.euw.leagueoflegends.com", "matchhistory.na.leagueoflegends.com")
                    .Replace("currentAccount%2FEUW1%2F", "currentAccount%2FNA1%2F")
                    .Replace("/#match-details/EUW1/", "/#match-details/NA1/")
                    .Replace("http://euw.leagueoflegends.com", "http://na.leagueoflegends.com");
        ...
        {
            int consoleWidth = getConsoleWidthSafe();
            drawLine(false);
            Console.WriteLine(String.Format("{0," + ((consoleWidth / 2) + Header1.Length / 2) + "}", Header1));
            Console.WriteLine(String.Format("{0," + ((consoleWidth / 2) + Header2.Length / 2) + "}", Header2));
            Console.WriteLine(String.Format("{0," + ((consoleWidth / 2) + Header3.Length / 2) + "}", Header3));
            drawLine(true);
        }
        ...
        private static void drawLine(bool _break)
        {
            ...
            {
                if (i >= consoleWidth && _break)
                {
                    Console.Write("\n");
                    return;
                }
                else if (i >= consoleWidth && !_break)
                {
                    Console.Write("\n");
                    return;
                }
